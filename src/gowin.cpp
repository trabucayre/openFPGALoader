...
bool Gowin::writeFLASH(uint32_t page, const uint8_t *data, int length)
{
	...
	uint8_t xpage[256];
	length /= 8;
	ProgressBar progress("Writing to FLASH", length, 50, _verbose);
...
bool Gowin::writeSRAM(const uint8_t *data, int length)
{
	...
	ProgressBar progress("Load SRAM", length, 50, _verbose);
...
bool Gowin::eraseFLASH()
{
	...
	ProgressBar progress("Erasing FLASH", 100, 50, _verbose);
